...
To ensure the reproducibility of our research, we provide the following guides:
## 4.1 Code Availability
The complete codes  for model training and evaluation are available at: https://github.com/X67-cat/GNNASM-main
## 4.2 Datasets
1. All datasets used in this study are included in this repository
2. The raw data sourced from NCBI PubChem BioAssay and CTD databases are publicly available, and our preprocessing scripts are provided in this repository.
## 4.3 Pretrained Models
1. We provide complete model training codes. The best-performing model can be directly generated by running our training script (GNNmodel/train_gnn.py).
2. The model architecture, hyperparameters, and training pipeline are fully defined in the codes. By setting the fixed random seed (seed=42), we ensure that identical models are generated in each round.
3. When users execute the training script, the generated model will be automatically saved to models/gcn_model_fold_7.h5, thereby reproducing all prediction results reported in our paper.
## 4.4 Environment Configuration:
1. Complete dependency is listed in requirements
2. 2.Detailed hardware and software specifications are provided in Section 2
...
2. All hyperparameters and model architectures are explicitly defined in the codes, We have verified that following these instructions, users can obtain results that are  identical to those reported in our paper. 
# 5 Results 
